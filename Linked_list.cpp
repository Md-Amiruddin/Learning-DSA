...
int getIntersection(Node* head1, Node* head2)  //returns the value of intersecting node or -1 if lists are not intersected.
{
    ...
    int count=1;
    while(count<=diff && ptr1!=NULL){   //setting the pointer1 of 1st list at an equal distance as pointer 2 of 2nd list from the intersection
        ptr1=ptr1->next;
        count++;
    }

    while(ptr1!=NULL && ptr2!=NULL){  //moving ahead in both lists until an intersection/NULL is found
        if(ptr1==ptr2){
            return ptr1->value;
        }
        ptr1=ptr1->next;
