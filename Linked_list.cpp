...
void evenAfterOdd(Node* &head)  //To place even position nodes after odd position nodes
{
    if(head==NULL){
        return;
    }
    ...
    while(odd->next!=NULL && even->next!=NULL){   //always check odd->next before even->next otherwise it will throw error in case if total no of nodes is odd.
        odd->next=even->next;
        odd=odd->next;
        even->next=odd->next;
        even=even->next;
    ...
    odd->next=evenStart;
    if(even!=NULL){          //always check this, otherwise program will show error. In case of odd total no of nodes, even pointer becomes NULL, acessing even->next will throw error.
        even->next=NULL;
    }
}
...
int main()
{
    ...
    Node* listHead = NULL;
    insertAtTail(listHead,11);
    insertAtTail(listHead,12);
    insertAtTail(listHead,13);
    insertAtTail(listHead,14);
    insertAtTail(listHead,15);
    insertAtTail(listHead,16);
    insertAtTail(listHead,17);

    evenAfterOdd(listHead);
    cout<<"\n";
    printList(listHead);
    return 0;
}